...
CANDIDATE_DIRS = [
    ...
    Path("/app/models"),                   # Container root
    Path("models"),                        # Working directory relative
]

MODEL_DIR = next(
    (p for p in CANDIDATE_DIRS if (p / "dropout_random_forest.pkl").is_file()),
    None
)

if MODEL_DIR is None:
    raise FileNotFoundError(
        "Dropout model artifacts not found. "
        f"Searched candidate folders: {[str(d) for d in CANDIDATE_DIRS]}"
    )

MODEL_PATH = MODEL_DIR / "dropout_random_forest.pkl"
FEATURES_PATH = MODEL_DIR / "dropout_feature_columns.pkl"
